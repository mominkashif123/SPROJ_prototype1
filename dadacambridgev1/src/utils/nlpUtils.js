...
const cosineSimilarity = (vecA, vecB) => {
    const vecA_reshaped = tf.reshape(vecA, [1, -1]);  // Ensure correct shape
    const vecB_reshaped = tf.reshape(vecB, [1, -1]);  // Ensure correct shape
...
export const checkSimilarity = async (studentAnswer, correctAnswers) => {
    await loadModel(); // Load the NLP model if not already loaded
